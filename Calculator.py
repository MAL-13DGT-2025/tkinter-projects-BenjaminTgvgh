import tkinter as tk  #This is needed 
...
b_x = ttk.Button(root, text = "x", command = lambda:press('x'))
b_x.grid(row = 3, column = 3)

b_CLR = ttk.Button(root, text = "CLR", command = press('CLR'))
b_CLR.grid(row = 4, column = 0)

b_0 = ttk.Button(root, text = "0", command = lambda:press('0'))
b_0.grid(row = 4, column = 1)

b_subtract = ttk.Button(root, text = "=", command = press('='))
